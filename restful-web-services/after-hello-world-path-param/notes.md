...
```
![](https://www.baeldung.com/wp-content/uploads/2016/08/SpringMVC.png)

[SO Answers on DispatcherServlet](https://stackoverflow.com/questions/2769467/what-is-dispatcher-servlet-in-spring)

[Good Read on DTO/DAO](https://stackoverflow.com/questions/35078383/what-are-the-dao-dto-and-service-layers-in-spring-framework)

- Who is configuring dispatcher servlet?

```text
Spring autoconfiguration configures the servet once it is found in the classpath. 
```

- What does dispatcher servlet do? 

```text
Dispatcher servlet knows where to send the request based on the path and the method it supports to perform that task. 
Looking at the log will show like this, which means that the dispatcherServlet is handling all the root "/" of the request. 

- Mapping servlet: 'dispatcherServlet' to [/]

Then it finds that the perfect match based on the request received and serve that accordingly. So we can say the method and the statements are
the handler here.  
...
```text
The response is returned because the @RestController contains both the @Controller and @ResponseBody hence the @ResponseBody 
will come into affect through the message-converter class/service. Here the message-converter is Jackson binder which will convert the Java Bean, 
i.e. here the "HelloWorldBean" as a JSON object.
```
- Who is configuring the error mapping?
```text
The answer is "Spring autoconfiguration" again. Spring finds the default error handler definition in the class path. 
Similar here the "errors" are mapped back to response though a BasicErrorController. We can anytime customize it according to our need.  
